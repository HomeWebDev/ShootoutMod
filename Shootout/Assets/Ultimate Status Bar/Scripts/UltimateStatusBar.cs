...
[ExecuteInEditMode]
[AddComponentMenu( "Ultimate UI/Ultimate Status Bar" )]
public class UltimateStatusBar : MonoBehaviour
{
	...
	public class UltimateStatus
	{
		...
		{
			// If the application is not playing, then no visiblity options should be applied.
	...
	void Awake ()
	{
		...
		for( int i = 0; i < UltimateStatusList.Count; i++ )
		{
			...
			// If the statusImage component is assigned, then update the color of it.
			//if( UltimateStatusList[ i ].statusImage != null )
			//	UltimateStatusList[ i ].UpdateStatusColor();
		...
		// If the user is is wanting to update the visibility of the Ultimate Status Bar using fade...
		if( updateVisibility != UpdateVisibility.Never && updateUsing == UpdateUsing.Fade )
		{
			...
			// If the user wants to update the visibility when the status has been updated, and the intial state is true, then start the countdown.
	...
	public IEnumerator SmoothFill ( UltimateStatus status )
	{
		...
		for( float t = 0.0f; t < 1.0f; t += Time.deltaTime * speed )
		{
			...
			// Call the UpdateStatusOptions each frame that this status is updated to udpate options.
	...
	/// <summary>
	/// Updates the default status on the Ultimate Status Bar.
	/// </summary>
	/// <param name="currentValue">The current value of the status.</param>
	...
	static public void UpdateStatus ( string statusBarName, string statusName, float currentValue, float maxValue )
	{
		...
		if( !GlobalStatusRegistered( statusBarName, statusName ) )
			return;
	...
	/// </summary>
	static bool GlobalStatusRegistered ( string statusBarName, string statusName )
	{
